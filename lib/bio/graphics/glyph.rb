...
attr_reader :glyphs
  ...
  @glyphs = [:generic, :directed, :transcript, :scale, :label, :histogram, :circle, :down_triangle, :up_triangle, :span, :protein]
  ...
  def self.protein(args)
    ...
    if not args[:domains].empty?
      args[:domains].each do |domain|
          composite <<  Bio::Graphics::Primitive.new(:rectangle, {
                                                  :x => domain[:pix_start],
                                                  :width => domain[:pix_width],
                                                  :y => args[:y],
                                                  :height => args[:height],
                                                  :fill_color => domain[:domain_fill_color],
                                                  :stroke =>args[:domain_stroke],
                                                  :stroke_width => args[:domain_stroke_width],
                                                  :style => args[:domain_style]} )
                                                end
